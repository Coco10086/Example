...
/// 4个角对称的Image
/// </summary>
[AddComponentMenu("UI/Tiled4Image", 11)]
public class Tiled4Image : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
{
	...
	void GenerateTiledSprite(VertexHelper toFill)
	{
		...
		v.color = color;
		float xMin = border.x;
		float xMax = tileWidth*2 - border.z;
		float yMin = border.y;
		float yMax = tileHeight*2 - border.w;
		...
		AddQuad(toFill, new Vector2(0, 0) + rect.position, new Vector2(tileWidth, tileHeight) + rect.position, color, uvMin, clipped, 0);
		AddQuad(toFill, new Vector2(0, 0) + rect.position, new Vector2(tileWidth, tileHeight) + rect.position, color, uvMin, clipped, 1);
		AddQuad(toFill, new Vector2(0, 0) + rect.position, new Vector2(tileWidth, tileHeight) + rect.position, color, uvMin, clipped, 2);
		AddQuad(toFill, new Vector2(0, 0) + rect.position, new Vector2(tileWidth, tileHeight) + rect.position, color, uvMin, clipped, 3);
	}
	...
	static void AddQuad(VertexHelper vertexHelper, Vector2 posMin, Vector2 posMax, Color32 color, Vector2 uvMin, Vector2 uvMax, int flag=0)
	{
		int startIndex = vertexHelper.currentVertCount;
		if(flag==0)
		{
			vertexHelper.AddVert(new Vector3(posMin.x, 0, 0), color, new Vector2(uvMin.x, uvMin.y));
			vertexHelper.AddVert(new Vector3(posMin.x, -posMin.y, 0), color, new Vector2(uvMin.x, uvMax.y));
			vertexHelper.AddVert(new Vector3(posMax.x, -posMin.y, 0),color, new Vector2(uvMax.x, uvMax.y));
			vertexHelper.AddVert(new Vector3(posMax.x, 0, 0), color, new Vector2(uvMax.x, uvMin.y));
		}
		else if(flag==1)
		{
			vertexHelper.AddVert(new Vector3(-posMin.x, 0, 0), color, new Vector2(uvMin.x, uvMin.y));
